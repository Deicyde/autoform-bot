...
class _FakeAsyncClient:
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.diagnostic_fatal_error = False
    ...
    async def diagnostics(self, path: str, *, fresh: bool, timeout: float):
        ...
        return SimpleNamespace(
            items=self.diagnostic_items,
            fatal_error=self.diagnostic_fatal_error,
        )
...
def test_lsp_backend_import_keeps_fastmcp_lazy(tmp_path: Path) -> None:
    result = subprocess.run(
        [
            ...
            "-c",
            (
                "import sys; import servers.lsp.server; "
...
def test_partial_start_retries_cleanup_without_dropping_ownership(
    tmp_path: Path,
    monkeypatch,
) -> None:
    project = tmp_path / "project"
    ...
    class PartiallyStartedClient(_FakeAsyncClient):
        async def start(self) -> None:
            self.calls.append(("start",))
            self.alive = True
            raise LeanTransportError("initialize failed", "watchdog failed")

    session = lsp.LeanLspSession(
        lsp.LspConfig(cwd=str(project), timeout=1),
        client_factory=PartiallyStartedClient,
    )
    process_group_id = 424242
    monkeypatch.setattr(session, "_read_process_identity", lambda: process_group_id)
    ...
    def terminate_process_group() -> None:
        cleanup_states.append(
            (
                session._client is not None,
                session._process_group_id,
                session._identity_path is not None,
                session._loop is not None,
            )
        )
        if len(cleanup_states) == 1:
            raise lsp.LspCleanupError("group still alive")

    monkeypatch.setattr(session, "_terminate_process_group", terminate_process_group)

    with pytest.raises(lsp.LspProtocolError, match="initialize failed") as raised:
        session.start()

    assert raised.value.stderr_tail == "watchdog failed"
    assert cleanup_states == [
        (True, process_group_id, True, True),
        (True, process_group_id, True, True),
    ]
    assert session._client is None
...
def test_failed_start_preserves_cancellation_after_verified_cleanup(
    tmp_path: Path,
    monkeypatch,
) -> None:
    project = tmp_path / "project"
    ...
    def submit(awaitable, timeout: float, operation: str):
        if operation == "starting Lean LSP":
            awaitable.close()
    ...
    def terminate_process_group() -> None:
        ...
        if cleanup_attempts == 1:
            raise lsp.LspCleanupError("retry cleanup")
...
def test_launcher_scrubs_lake_environment_and_records_own_group(tmp_path: Path) -> None:
    ...
    shadow_package = shadow_root / "servers"
    shadow_package.mkdir(parents=True)
    (shadow_package / "__init__.py").write_text(
        'raise SystemExit("target project shadowed Autoform")\n',
        encoding="utf-8",
    )
    probe = (
        "import json, os, sys; "
...
def test_diagnostics_surface_fatal_file_progress(fake_session) -> None:
    session, source, client = fake_session
    client.diagnostic_items = []
    client.diagnostic_fatal_error = True
...
def test_leanclient_failure_poisons_session_before_next_operation(fake_session) -> None:
    session, source, client = fake_session
    client.diagnostics_error = LeanTransportError(
        "broken stream",
        "uncaught exception: declaration has metavariables",
    )

    with pytest.raises(lsp.LspProtocolError, match="broken stream") as raised:
        session.get_diagnostics(str(source))

    assert raised.value.stderr_tail == (
        "uncaught exception: declaration has metavariables"
    )
    assert "Lean stderr tail" in str(raised.value)
    assert "declaration has metavariables" in str(raised.value)
    assert not session.is_alive()
...
def test_close_escalates_until_the_supervised_group_is_gone(
    ...
    monkeypatch.setattr(
        session,
        "_process_group_has_live_members",
        lambda process_group_id: group_alive["value"],
    )
...
def test_zombie_only_process_group_is_treated_as_terminated(monkeypatch) -> None:
    session = lsp.LeanLspSession(lsp.LspConfig())
    process_group_id = 444444
    zombie = SimpleNamespace(
        info={"pid": process_group_id, "status": lsp.psutil.STATUS_ZOMBIE}
    )
    monkeypatch.setattr(session, "_process_group_exists", lambda pgid: True)
    monkeypatch.setattr(lsp.psutil, "process_iter", lambda attrs: [zombie])
    monkeypatch.setattr(lsp.os, "getpgid", lambda pid: process_group_id)

    assert not session._process_group_has_live_members(process_group_id)


def test_darwin_eperm_without_visible_members_is_treated_as_terminated(
    monkeypatch,
) -> None:
    session = lsp.LeanLspSession(lsp.LspConfig())

    def inaccessible_group(process_group_id: int, signal_number: int) -> None:
        raise OSError(lsp.errno.EPERM, "operation not permitted")

    monkeypatch.setattr(lsp.os, "killpg", inaccessible_group)
    monkeypatch.setattr(lsp.psutil, "process_iter", lambda attrs: [])

    assert not session._process_group_has_live_members(464646)


def test_live_process_group_member_is_not_mistaken_for_darwin_zombie(
    monkeypatch,
) -> None:
    session = lsp.LeanLspSession(lsp.LspConfig())
    process_group_id = 454545
    member = SimpleNamespace(
        info={"pid": process_group_id, "status": lsp.psutil.STATUS_SLEEPING}
    )
    monkeypatch.setattr(session, "_process_group_exists", lambda pgid: True)
    monkeypatch.setattr(lsp.psutil, "process_iter", lambda attrs: [member])
    monkeypatch.setattr(lsp.os, "getpgid", lambda pid: process_group_id)

    assert session._process_group_has_live_members(process_group_id)
...
def test_document_operations_are_serialized(fake_session) -> None:
    ...
    async def diagnostics(path: str, *, fresh: bool, timeout: float):
        ...
        if call_count == 1:
            ...
            second_entered.set()
        return SimpleNamespace(items=[], fatal_error=False)
